...
for taxcode in [9606, 10090]:
    es = omnipath.db.get_db('enz_sub', ncbi_tax_id=taxcode)
    es.make_df()
    ...
    sources = ['PhosphoSite_MIMP', 'PhosphoSite_ProtMapper'] if taxcode == 9606 else ['PhosphoSite']
    ...
    id_mapping = pd.read_csv(f'../ksea/id_conversion_{taxcode}.txt')
    fr = 'uniprot'
    to = 'gene_name'
    ...
    with open(f'../ksea/psp_kinase_substrate_adjacency_matrix_{taxcode}.csv', 'w') as f:
