...
CYTOSCAPE_PATH = '/home/jm/Cytoscape_v3.9.1/Cytoscape'
...
def run_cytoscape(filepath: Path) -> Path:
    ...
    #TODO: Maybe not necessary anymore, see run_phonemes script
    def row_contains_phosphosite(row):
        return ('_' in row['Node1'] and '__' not in row['Node1']) or ('_' in row['Node2'] and '__' not in row['Node2'])

    phonemes_df_proteins = phonemes_df[~phonemes_df.apply(row_contains_phosphosite, axis=1)]
    ...
    network = phonemes_df_proteins.rename(
    ...
    p4c.export_network(filename=output_path, type='CX', overwrite_file=True)
    return output_path
